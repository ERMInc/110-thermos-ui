...
(let [diversity (net-diversity/diversity-factor {})]
  (defn- evaluate-alternative [alternative kwh kwp-space kwp-water m2 connections fuel-prices]
    (let [{:keys [capex-fixed capex-per-kwh capex-per-kwp capex-per-m2 capex-per-connection
                  opex-fixed opex-per-kwh opex-per-kwp opex-per-m2 opex-per-connection
                  repex-fixed repex-per-kwh repex-per-kwp repex-per-m2 repex-per-connection repex-interval
           ...
           ;; TODO comment out in normal use?
           :or
           {capex-fixed 0.0 capex-per-kwh 0.0 capex-per-kwp 0.0 capex-per-m2 0.0 capex-per-connection 0.0
            opex-fixed 0.0 opex-per-kwh 0.0 opex-per-kwp 0.0 opex-per-m2 0.0 opex-per-connection 0.0
            repex-fixed 0.0 repex-per-kwh 0.0 repex-per-kwp 0.0 repex-per-m2 0.0 repex-per-connection 0.0 repex-interval 12
            oversize-kw 0.0
            round-size :exact}
          ...
          fuel-prices (get fuel-prices fuel)

          ;; we add on 0.0 at the front because
          ;; we don't incur any opex or fuelex in year 0.
          
          fuel-cost-per-kwh
          (vec (conj (for [[_ price] fuel-prices] (/ price efficiency)) 0.0))

          fuel-cost-standing
          (vec (conj (for [[sc _] fuel-prices] sc) 0.0))

          kwp (+ (if (= size-for :space-heat) kwp-space kwp-water)
                 oversize-kw)
          ...
          kwp (case round-size
                ...
                :up    (Math/ceil kwp)
                :down  (Math/floor kwp)
                :half-even (Math/round kwp))
          

          capex (+ capex-fixed
                   (* capex-per-connection connections)
                   (* capex-per-m2 m2)
                   (* capex-per-kwh kwh)
                   (* capex-per-kwp kwp))

          opex (+ opex-fixed
                  (* opex-per-connection connections)
                  (* opex-per-m2 m2)
                  (* opex-per-kwh kwh)
                  (* opex-per-kwp kwp))

          repex (+ repex-fixed
                   (* repex-per-connection connections)
                   (* repex-per-m2 m2)
                   (* repex-per-kwh kwh)
                   (* repex-per-kwp kwp))
          ...
          cost (finance/pv
                (finance/sum-series
                 ...
                 (finance/up-front (+ capex-fixed
                                      (* capex-per-connection connections)
                                      (* capex-per-m2 m2)))
                 (finance/recurring repex-interval (+ repex-fixed
                                                      (* repex-per-connection connections)
                                                      (* repex-per-m2 m2)))
                 (finance/annual (+ opex-fixed
                                    (* opex-per-connection connections)
                                    (* opex-per-m2 m2)))))
          ...
          cost%kwh (finance/pv
                    (finance/sum-series
                     (finance/up-front capex-per-kwh)
                     (finance/recurring repex-interval repex-per-kwh)
                     (finance/annual opex-per-kwh)
                     fuel-cost-per-kwh))

          cost%kwp (finance/pv
                    (finance/sum-series
                     (finance/up-front capex-per-kwp)
                     (finance/recurring repex-interval repex-per-kwp)
                     (finance/annual opex-per-kwp)))
          ]
      {:capex       capex
       :opex        opex
       :repex       repex
       :fuel        (+ (* kwh (reduce + 0.0
                                      (take finance/*discount-period* fuel-cost-per-kwh)))
                       (reduce + 0.0 (take finance/*discount-period* fuel-cost-standing)))
       :cost        cost
       :cost%kwh    cost%kwh
       :cost%kwp    cost%kwp
       })))
   ...
   (let [fuel-prices    (::document/fuel-prices instance)
     ...
     (for [id ids
       ...
       (let [kwh (candidate/annual-demand candidate (document/mode instance))
             ...
             cost-terms (evaluate-alternative
                         alternative
